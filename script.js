...
function revealOnScroll() {
  ...
  revealElements.forEach(el => {
    ...
    const revealPoint = 150; // how far from bottom to trigger
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function type() {
    if (!isDeleting) {
      ...
      if (index === 0) {
        isDeleting = false;
        speed = 600; // pause before retyping
      } else {
        speed = 100;
      }
    }
...
function fillSkills() {
  skillSpans.forEach(span => {
    const width = span.getAttribute('style'); // get width %
    span.style.width = '0';
    setTimeout(() => {
...
function animateSkills() {
  ...
  if(sectionPos < screenPos - 100) { // 100px before fully in view
    skillSpans.forEach(span => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const slides = carouselContainer.querySelectorAll('.carousel .slide, .carousel .slide-active');
  let currentSlide = 0;
...
function autoSlide() {
  ...
  slides.forEach((slide, i) => slide.style.display = i === autoIndex ? 'block' : 'none');
}
